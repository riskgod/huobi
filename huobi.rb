...
def sign_a_b(x1, x2, y1, y2)
  a = (y2-y1)/(x2-x1)
  b = y2-a*x2
  return a, b
end

access_key = '10caa14b-a1f4d5fa-4c424b51-de1ac'
secret_key = '292ab86a-56df23eb-ccbdb674-a6e5b'
account_id = '867777'
huobi_pro = HuobiPro.new(access_key,secret_key,account_id)

low_array = []
high_array = []
huobi = huobi_pro.history_kline('ethusdt', '15min' )["data"]
huobi.each{|x| low_array.push(x["low"])};
huobi.each{|x| high_array.push(x["high"])};

high_point_1 = high_array.sort[-1]
high_point_2 = high_array.sort[-2]
low_point_1 = low_array.sort[0]
low_point_2 = low_array.sort[1]
